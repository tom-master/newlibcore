...
namespace NewLibCore.Data.SQL.Mapper.Handler
{
    ...
    internal class UpdateHandler<TModel> : HandlerBase where TModel : EntityBase, new()
    {
        ...
        protected override ExecuteResult Execute()
        {
            ...
            ParserResult.Append(String.Format(Template.UpdateTemplate, tableName, aliasName, String.Join(",", propertys.Select(p => $@"{aliasName}.{p.Key}=@{p.Key}"))), propertys.Select(c => new MapperParameter(c.Key, c.Value)));
