...
### 简介
**EntityMapper**是一个可以将简单的表达式树翻译为对应的SQL语句的类库，支持**单表查询**，**多表连接查询**，**分页查询**，**原生SQL执行**和**排序**，**外建表**等基本操作，后续还会继续支持**聚合函数**等翻译操作。    
支持数据库:SqlServer2008及更高版本,Mysql 6.0及更高版本
# 数据实体变化
* 特性
...
# 基本操作示例
  ...
  using(var mapper = EntityMapper.CreateMapper())
  {
      ...
      //查询指定的导航属性
      var result = mapper.Query<User>().Include(d=>d.Config).ToList();
  }
  ```
  5. 事物操作
  ```C#
